...
public class Transaction {
    ...
    public Transaction(User recipient, User sender, TypeTransfer type, double amountTransfer) {
        ...
        else if (sender.getBalance() < Math.abs(amountTransfer))
            throw new IllegalTransactionException("Недостаточно средств для совершения перевода");
        else {
            setRecipient(recipient);
            setSender(sender);
            setTypeTransfer(type);
            setAmountTransfer(amountTransfer);
            if (type == TypeTransfer.CREDIT) recipient.setBalance(recipient.getBalance() - Math.abs(amountTransfer));
            else sender.setBalance(sender.getBalance() + Math.abs(amountTransfer));
        }
    }
