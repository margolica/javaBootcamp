...
public class Transaction {
    ...
    public Transaction(User recipient, User sender, TypeTransfer type, double amountTransfer) {
        if ((type == TypeTransfer.DEBIT && amountTransfer > 0) || (type == TypeTransfer.CREDIT && amountTransfer < 0))
            ...
            if (type == TypeTransfer.CREDIT && recipient.getBalance() > Math.abs(amountTransfer))
                recipient.setBalance(recipient.getBalance() + amountTransfer);
            else if (!(recipient.getBalance() > Math.abs(amountTransfer)))
                throw new IllegalTransactionException("Недостаточно средств для совершения перевода");
            else sender.setBalance(sender.getBalance() + amountTransfer);
        }
    }
